...
class UsageReportCounters:
    ...
    # bytes under more than one tier. All tier bytes also flow into logs_mb_in_period; the sum of
    # all three tiers should equal logs_mb_in_period and serves as a belt-and-braces cross-check.
...
def get_teams_with_logs_retention_bytes_in_period(
    ...
    `logs_mb_in_period`; the sum of all three tiers should equal it and serves as a belt-and-braces
    cross-check.
    """
    with tags_context(product=Product.LOGS, feature=Feature.USAGE_REPORT):
        rows = sync_execute(
