...
def _resolve_reviewer_mentions(slack: SlackIntegration, reviewer_users: list[User]) -> list[str]:
    # `<@U…>` mention when the reviewer's email resolves in this workspace, else plain name.
    mentions: list[str] = []
    for user in reviewer_users[:_MAX_REVIEWER_MENTIONS]:
        ...
        mentions.append(f"<@{slack_user_id}>" if slack_user_id else _posthog_user_display_name(user))
    return mentions
...
def _signals_field_value(report: SignalReport) -> str:
    if not report.signal_count:
        return ""
    label = "signal" if report.signal_count == 1 else "signals"
    value = f"{report.signal_count} {label}"
    if report.total_weight:
        value = f"{value}  ·  weight {report.total_weight:g}"
    return value
...
def _build_message_blocks(
    ...
    *,
    ...
    # Mentions go in the mrkdwn body, not the header — Slack only resolves `<@U…>` in mrkdwn.
    title_line = report.title or "New signals inbox item"

    header_text = ":inbox_tray:  *Inbox item*"
    if priority:
        header_text = f"{header_text}  ·  {priority}"

    body_parts = [f"*{title_line}*"]
    if reviewer_mentions:
        body_parts.append(" ".join(reviewer_mentions))
    ...
    signals_value = _signals_field_value(report)
    if signals_value:
        fields.append({"type": "mrkdwn", "text": f"*Signals*\n{signals_value}"})
    if fields:
