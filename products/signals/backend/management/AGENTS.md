...
python manage.py sync_signals_scout_skills --all-enabled --force
```
...
**Rollout-flag gate.** By default this command honors the same `signals-scout` feature flag
the Temporal coordinator uses — a team that is `enabled=True` but flag-gated off is reported
as `team N: skipped — gated by signals-scout rollout flag` and not synced. Use `--force` for
emergency reverts (e.g. pushing a fixed canonical to every team regardless of rollout state)
or for local dev/testing. See `../scout_harness/feature_flags.py` for the flag's contract
